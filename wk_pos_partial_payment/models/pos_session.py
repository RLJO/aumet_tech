    ...
    def _accumulate_amounts(self, data):
        ...
        for order in self.order_ids:
            ...
            if order.is_invoiced:
                ...
                credit_amounts[
                    'partner'] = order.partner_id.parent_id if order.partner_id.parent_id  else order.partner_id
        ...
        for receivable_account_id, amounts in data.get('wk_invoice_receivables', {}).items():
            for item in amounts:
                ...
                invoice_receivable_vals[receivable_account_id].append(self._get_invoice_receivable_vals(
                    receivable_account_id, item['amount'], item['amount_converted'], partner=partner))
    ...
    def _reconcile_account_move_lines(self, data):
        ...
        for statement in self.statement_ids:
            ...
            all_lines = (
                    ...
                    | combine_cash_statement_lines[statement].mapped('move_id.line_ids').filtered(
                lambda aml: aml.account_id.internal_type == 'receivable')
                    | split_cash_receivable_lines[statement]
                    | combine_cash_receivable_lines[statement]
            )
            _logger.info("***info of lines*%r****%r*****%r****%r",
                         split_cash_statement_lines[statement].mapped('move_id.line_ids').filtered(
                             lambda aml: aml.account_id.internal_type == 'receivable'),
                         combine_cash_statement_lines[statement].mapped(
                             'move_id.line_ids').filtered(lambda aml: aml.account_id.internal_type == 'receivable'),
                         split_cash_receivable_lines[statement], combine_cash_receivable_lines[statement])
        ...
        for account_id in order_account_move_receivable_lines:
            _logger.info("*************111111111***********:%r", (order_account_move_receivable_lines[account_id]
                                                                  | invoice_receivable_lines.get(account_id, self.env[
                        'account.move.line'])
                                                                  ).with_context(coming_from_pos=True).read(
                ['reconciled']))
