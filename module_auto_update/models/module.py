...
ABOUT_TO_UPGRADE_MODULE = "module_auto_update.modules_to_be_upgraded"
...
class Module(models.Model):
    ...
    @api.model
    def _get_last_upgraded_modules(self):
        Icp = self.env["ir.config_parameter"]
        return Icp.get_param(ABOUT_TO_UPGRADE_MODULE, '[]')

    @api.model
    def _save_last_upgraded_modules(self,modules):
        Icp = self.env["ir.config_parameter"]
        Icp.set_param(ABOUT_TO_UPGRADE_MODULE,modules)
        # Icp.flush()
    ...
    def upgrade_changed_checksum(self, overwrite_existing_translations=False):
        ...
        if (not changed_modules and not self._get_modules_partially_installed() ) or set(changed_modules.mapped("name")) ==set(ast.literal_eval(self._get_last_upgraded_modules())):
            _logger.info(
        ...
        self._save_last_upgraded_modules(changed_modules.mapped("name"))
        self.env.cr.commit()
        ...
        self._save_last_upgraded_modules('[]')
        self.env.cr.commit()  # pylint: disable=invalid-commit
