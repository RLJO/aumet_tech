odoo.define('stock_barcode.ClientAction', function (require) {
    ...
    var _t = core._t;
    BarcodeEvents.inputTimeOut = 12000;
    ...
    codeReader
        ...
        .then(function (result) {
            ...
            _.each(result, function (item) {
                $('#js_id_sh_stock_move_barcode_mobile_cam_select').append($('<option>', {
                    value: item.deviceId,
                    text: item.label,
                }));
            });
    ...
    var ClientAction = AbstractAction.extend({
        custom_events: {
            ...
            save_line: '_save',
            change_virtual_id: '_change_virtual_id',
            open_camera: '_opencamera',
            stop_camera: '_stopcamera',
        },
        ...
        decodeOnce: function (codeReader, selectedDeviceId) {
            codeReader
                ...
                .then((result) => {
                    this._stopcamera();
                    this._onBarcodeScannedHandler(result.text);
                })
                .catch((err) => {
                    console.error(err);
                });
        ...
        _opencamera: function (ev) {
            ...
            $("#js_id_sh_stock_move_barcode_mobile_start_btn").hide();
        ...
        _stopcamera: function (ev) {
            ...
            $("#js_id_sh_stock_move_barcode_mobile_start_btn").show();
        },
        ...
        _isProduct: function (barcode) {
            var self = this;
            var res = {}
            var parsed_barcode = this.barcodeParser.parse_barcode(barcode);
            _.forEach(parsed_barcode, function (parsed) {
                if (parsed.type === 'weight') {
                    var product = self.productsByBarcode[parsed.base_code];
                    ...
                    res.product = self.productsByBarcode[parsed.code];
                } else if (parsed.type === 'expiration_date') {
                    res.expiration_date = parsed.value;
                } else if (parsed.type === 'lot') {
                    res.lot = parsed.value;
                }
            });
            return res;
        },
        ...
        _change_virtual_id: function (event) {
            var selectedLineJquery = event.data.line[0];
            var virtual_id = event.data.virtual_id;
            if (virtual_id in this.line_mapping_ids) {
                selectedLineJquery.attr('data-id', this.line_mapping_ids[virtual_id])
            }
            return Promise.resolve();
        },
        ...
        _incrementLines: function (params) {
            ...
            if (line) {
                ...
                if (params.product.tracking === 'none' ||
                    ...
                    line = this._makeNewLine(params.product, params.barcode, 1, params.package_id, params.result_package_id);
                }
        ...
        _step_product: function (barcode, linesActions) {
            ...
            this.currentStep = 'product';
            var errorMessage;
            var parsed_product = this._isProduct(barcode);
            var product = parsed_product.product;
            var lot = parsed_product.lot || undefined;
            var lot_expiry = parsed_product.expiration_date || undefined;
            if (product) {
                ...
                if (res.isNewLine) {
                    if (this.actionParams.model === 'stock.inventory') {
                        ...
                        if (lot_expiry !== undefined && lot !== undefined) {
                            res.lineDescription.prod_lot_id = [0, 0, {
                                'name': lot,
                                'product_id': product.id,
                                'company_id': self.currentState.company_id[0],
                                'expiration_date': lot_expiry
                            }]
                        }
                        return this._rpc({
                            model: 'product.product',
