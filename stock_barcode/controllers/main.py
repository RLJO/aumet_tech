...
class StockBarcodeController(http.Controller):
    ...
    def get_lot_id(self, write_vals):
        vals = write_vals[0][2]
        lot_vals = vals['prod_lot_id'][2]
        lot_id = request.env['stock.production.lot'].search([('name', '=', lot_vals['name']), (
            'product_id', '=', lot_vals['product_id'])])
        if not lot_id:
            lot_vals['expiration_date'] = datetime.strptime(lot_vals['expiration_date'].split('.')[0].replace('T', ' '),
                                                            DEFAULT_SERVER_DATETIME_FORMAT)
            lot_id = request.env['stock.production.lot'].create(lot_vals)
        return lot_id
    ...
    def get_set_barcode_view_state(self, model_name, record_id, mode, write_field=None, write_vals=None):
        if mode != 'read':
            if 'prod_lot_id' in write_vals[0][2].keys() and type(write_vals[0][2].get('prod_lot_id')) == list and len(
                    write_vals[0][2].get('prod_lot_id')) > 2:
                lot_id = self.get_lot_id(write_vals)
                write_vals[0][2]['prod_lot_id'] = lot_id.id
