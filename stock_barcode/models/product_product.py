...
class Product(models.Model):
    ...
    def get_all_products_by_barcode(self):
        products = self.env['product.product'].search_read(
            ...
            ['barcode', 'display_name', 'uom_id', 'tracking', 'lst_price', 'multi_barcode_ids']
        )
        ...
        multi_barcode_ids = [product['multi_barcode_ids'][0] for product in products if product['multi_barcode_ids']]
        multi_barcodes = self.env['multi.barcodes'].browse(multi_barcode_ids)
        multi_barcode_products = [product.copy() for product in products if
                                  product['barcode'] in multi_barcodes.mapped('product_id').mapped('barcode')].copy()
        for mbp in multi_barcode_products:
            mbp['barcode'] = multi_barcodes.filtered(lambda p: p.product_id.barcode == mbp['barcode']).name
        products += multi_barcode_products

        products_to_read = self.env['product.product'].browse(list(set(t[1] for t in to_read))).sudo().read(
            ['display_name', 'uom_id', 'tracking', 'lst_price'])
        ...
        return {product['barcode']: product for product in products + to_add}
