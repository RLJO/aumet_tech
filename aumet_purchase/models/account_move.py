...
'''
    The override of these function was made 
    to edit the calculation of discount in case the partner is sub-agent
'''
    ...
    def _recompute_tax_lines(self, recompute_tax_base_amount=False):
        ...
        def _compute_base_line_taxes(base_line):
            ...
            if move.is_invoice(include_receipts=True):
                ...
                if base_line.partner_id.is_agent and base_line.discount != 0:  # Sub-Agent
                    price_unit_wo_discount = sign * base_line.price_unit / (1 + base_line.discount / 100)
                else:
...
class AccountMoveLine(models.Model):
    ...
    def _get_price_total_and_subtotal_model(self, price_unit, quantity, discount, currency, product, partner, taxes,
                                            move_type):
        res = super(AccountMoveLine, self)._get_price_total_and_subtotal_model(price_unit=price_unit, quantity=quantity,
                                                                               discount=discount, currency=currency,
                                                                               product=product, partner=partner,
                                                                               taxes=taxes,
                                                                               move_type=move_type)
        if partner and partner.is_agent and discount != 0:
            # Compute 'price_subtotal'.
            line_discount_price_unit = price_unit / (1 + discount / 100)
