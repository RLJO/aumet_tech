odoo.define('flexipharmacy.ReturnOrderFetcher', function (require) {
    'use strict';
    ...
    class ReturnOrderFetcher extends EventBus {
        constructor() {
            super();
            this.currentPage = 1;
            this.ordersToShow = [];
        ...
        async _getOrderIdsForCurrentPage(limit, offset) {
            ...
            if (this.searchDomain){
                for (let [field, _, searchWord] of (this.searchDomain || []).filter((item) => item !== '|')) {
                    this.searchKeyWord = searchWord.substring(1, searchWord.length - 1);
                }
            }
            var order_ids = await this.rpc({
                model: 'pos.order',
                method: 'search_order_ids_from_product',
                kwargs: { config_id: this.configId, domain: this.searchDomain ? this.searchDomain : [], searchWord: this.searchKeyWord, limit, offset },
