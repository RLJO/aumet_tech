        ...
        confirm(){
            ...
            if (this.state.GiftVoucherNumber) {
                ...
                rpc.query(params, {async: false}).then(function(res){
                    if(res.length > 0){
                        if (res[0]){
                            var expiry_date = moment(res[0]['expiry_date']).locale('en').format('YYYY-MM-DD');
                            ...
                            if(res[0]['expiry_date'] && today > expiry_date){
                                self.state.GiftVouchermsg = "Gift Voucher is expired on "+moment(res[0]['expiry_date']).locale('en').format('DD. MMM. YYYY')
                    ...
                    if(res.length > 0){
                        if (res[0]){
                            ...
                            if(res[0]['expiry_date'] && today > expiry_date){
                                ...
                                self.state.GiftVouchermsg = "Gift Voucher Amount is "+self.env.pos.format_currency(res[0]['voucher_amount'])
                            }
                        }
                    }else{
                        self.state.GiftVouchermsg = "Invalid Gift Voucher code"
